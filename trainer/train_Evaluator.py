...
class Eva_Trainer():
    def __init__(self, config):
        ...
        self.logger = Logger(config['name'],config['port'],config['n_epochs'], len(self.dataloader), epoch = config['epoch'])
        self.loge = []
    def train(self):
        loge = self.loge
        ###### Training ######
        for epoch in range(self.config['epoch'], self.config['n_epochs']+1):
            ...
            for A1_noise,A2, A1_origin, Label in self.dataloader:
                A1_noise = cyclegan_remap(A1_noise[0][0]).unsqueeze(0).unsqueeze(0)
                A1_noise = A1_noise.cuda()
                A2 = A2.cuda()
                A1_origin = A1_origin.cuda()
                Label = Label.cuda()
                ...
                self.logger.log({'loss_E': sum(loge)/len(loge),},
                           images={'A1_origin':A1_origin[0,:,int(Depth/2),:,:], 'A1_noise': A1_noise[0,:,int(Depth/2),:,:], 'A2': A2[0,:,int(Depth/2),:,:], 'Label': Label[0,:,int(Depth/2),:,:],'pred':pred[0,:,int(Depth/2),:,:],                                            })
